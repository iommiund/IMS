...
class inventory
{
    ...
    public function createInstallOrder($customerId,$street,$town,$resourceTypeId,$uid){

        //Initiate create order, if failed redirect with failure, els redirect with success
        if (!db::getInstance()->query("insert into ims.orders (order_type_id,order_status_id,customer_id,street_id,town_id,resource_type_id,initiation_timestamp,initiation_uid) values (4,2,{$customerId},{$street},{$town},{$resourceTypeId},NOW(),{$uid})")) {
    ...
    public function cancelOrder($orderId,$uid,$customerId) {
        ...
        if (!db::getInstance()->query("update ims.orders o set o.order_status_id = 3, o.closing_uid = {$uid}, o.closing_timestamp = now() where o.order_id = {$orderId}")) {
            ...
            redirect::to('viewOrders.php?id=' . $customerId . '&cancelled');
        }
